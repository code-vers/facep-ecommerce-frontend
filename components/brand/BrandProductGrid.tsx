  ...
  return (
    <div className="flex-1 flex flex-col gap-6">
      ...
      <div className="border-b border-[#E5E5E6] pb-4 flex flex-col sm:flex-row sm:items-baseline justify-between gap-2">
        <div>
          <h3 className="text-[20px] font-bold text-black">
            Explore Plant House's Products
          </h3>
      ...
      {products.length > 0 ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 bg-white border border-[#E5E5E6] rounded-[4px] shadow-2xs">
          <ShoppingBag size={48} className="text-gray-300 mb-4" />
          <p className="text-[16px] font-semibold text-gray-600">No products match your filters.</p>
          <button
            type="button"
            onClick={onClearFilters}
            className="mt-4 px-4 py-2 bg-emerald-800 text-white rounded-[2px] text-[14px] font-bold hover:bg-emerald-950 transition-colors cursor-pointer"
          >
            Clear All Filters
          </button>
        </div>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-2 mt-8 border-t border-[#E5E5E6] pt-6">
          <button
            ...
            className="flex items-center gap-1 px-3 py-1.5 rounded-[2px] border border-[#E5E5E6] bg-white hover:bg-gray-50 text-[14px] text-gray-700 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer transition-colors"
          >
            <ChevronLeft size={16} />
            <span>Previous</span>
          </button>
          ...
          <div className="flex items-center gap-1">
            {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
              <button
                ...
                className={cn(
                  "size-10 flex items-center justify-center rounded-[2px] border text-[14px] cursor-pointer transition-all",
                  currentPage === page
                    ? "bg-emerald-800 border-emerald-800 text-white font-bold"
                    : "bg-white border-[#E5E5E6] text-gray-700 hover:bg-gray-50"
                )}
              >
                {page}
              </button>
