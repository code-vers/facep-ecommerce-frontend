...
export default function ProductDetailsStep() {
  ...
  return (
    <div className="border border-[#e5e5e6] border-solid bg-white flex flex-col items-start w-full relative shrink-0">
      <div className="flex flex-col gap-6 items-start p-4 md:p-[24px] w-full relative shrink-0">
        ...
        <div className="border border-[#e5e5e6] border-solid rounded-[4px] flex flex-col gap-[24px] items-start p-4 w-full relative shrink-0">
          <div className="flex flex-row items-center justify-between w-full">
            ...
            <button 
              onClick={addRow}
              className="font-normal text-[14px] md:text-[16px] text-[#165dd0] flex items-center hover:underline"
            >
          ...
          <div className="flex w-full relative shrink-0 overflow-x-auto">
            {/* Table Container */}
            <div className="flex w-full min-w-[600px] border-l border-t border-[#e5e5e6]">
              
              {/* Specification Name Column */}
              <div className="flex flex-col flex-1 border-r border-[#e5e5e6]">
                <div className="bg-[#f2f2f3] border-b border-[#e5e5e6] h-[34px] flex items-center px-2 py-2">
                  <span className="font-normal text-[14px] text-black">Specification Name</span>
                </div>
                {specs.map((spec) => (
                  <div key={`name-${spec.id}`} className="border-b border-[#e5e5e6] h-[48px] flex items-center px-2">
                    <input 
                      type="text" 
                      value={spec.name}
                      onChange={(e) => updateRow(spec.id, 'name', e.target.value)}
                      placeholder="Specification Name"
                      className="w-full bg-white border border-[#e5e5e6] rounded-[2px] h-[32px] px-3 py-2 text-[12px] text-black outline-none placeholder:text-[#848995]"
                    />
                  </div>
                ))}
              </div>
              ...
              <div className="flex flex-col flex-1 border-r border-[#e5e5e6]">
                ...
                {specs.map((spec) => (
                  <div key={`value-${spec.id}`} className="border-b border-[#e5e5e6] h-[48px] flex items-center px-2">
                    <input 
                      type="text" 
                      value={spec.value}
              ...
              <div className="flex flex-col w-[134px] border-r border-[#e5e5e6]">
                <div className="bg-[#f2f2f3] border-b border-[#e5e5e6] h-[34px] flex items-center justify-center px-2 py-2">
                  <span className="font-normal text-[14px] text-black">Action</span>
                </div>
                {specs.map((spec) => (
                  <div key={`action-${spec.id}`} className="border-b border-[#e5e5e6] h-[48px] flex items-center justify-center gap-3 px-2">
                    <button className="text-[#42454D] hover:text-black transition-colors">
                      <PenLine size={16} />
                    </button>
                    <button 
                      onClick={() => removeRow(spec.id)}
                      className="text-[#42454D] hover:text-red-600 transition-colors"
                    >
                      <Trash2 size={16} />
                    </button>
                  </div>
        ...
        {/* Returns Section */}
        <div className="flex flex-col md:flex-row gap-4 md:gap-6 w-full pt-4 mt-2 border-t border-[#e5e5e6]">
          <div className="flex flex-col gap-2 flex-[0.33] w-full">
            <p className="font-normal leading-[1.2] text-[16px] text-black">
              Return Policy
            </p>
          ...
          <div className="flex flex-col gap-2 flex-[0.67] w-full">
            <p className="font-normal leading-[1.2] text-[16px] text-black">
              Return terms
            </p>
            <div className="border border-[#e5e5e6] bg-white flex items-start px-3 py-2.5 rounded-sm w-full">
              <textarea
        ...
        <div className="w-full pt-4 mt-2 border-t border-[#e5e5e6]">
