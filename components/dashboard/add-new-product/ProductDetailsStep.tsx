...
import { ISpecification, useProductFormStore } from '../../../store/useProductFormStore';
...
export default function ProductDetailsStep() {
  ...
  const addRow = () => {
    const newRow: ISpecification = {
      id: Date.now().toString(),
      name: '',
      value: '',
    };
    store.setField('specifications', [...store.specifications, newRow]);
  };

  const removeRow = (id: string) => {
    store.setField(
      'specifications',
      store.specifications.filter((spec) => spec.id !== id),
    );
  };
  ...
  const updateRow = (id: string, field: 'name' | 'value', val: string) => {
    store.setField(
      'specifications',
      store.specifications.map((spec) => (spec.id === id ? { ...spec, [field]: val } : spec)),
    );
  };
      ...
      <div className='flex flex-col gap-[24px] items-start p-[24px] w-full relative shrink-0'>
          ...
          <div className='flex w-full relative shrink-0 overflow-x-auto'>
            ...
            <div className='flex w-full min-w-[600px] border-l border-[#e5e5e6]'>
              ...
              <div className='flex flex-col flex-1 border-r border-[#e5e5e6]'>
                ...
                {store.specifications.map((spec) => (
                  <div
                    key={`name-${spec.id}`}
                    className='border-b-[0.5px] border-[#e5e5e6] h-[48px] flex items-center px-[8px] py-[4px]'
                  >
                    <input
                      type='text'
                      value={spec.name}
                      onChange={(e) => updateRow(spec.id, 'name', e.target.value)}
                      placeholder='e.g. Ear Placement'
                      className='w-full bg-transparent text-[12px] text-[#42454d] outline-none placeholder:text-[#848995]'
                    />
                  </div>
                ))}
                {store.specifications.length === 0 && (
                  <div className='border-b-[0.5px] border-[#e5e5e6] h-[48px] flex items-center justify-center px-[8px] py-[4px]'>
                    <span className='text-[12px] text-gray-400'>No specifications added</span>
                  </div>
                )}
              </div>
              ...
              <div className='flex flex-col flex-1 border-r border-[#e5e5e6]'>
                ...
                {store.specifications.length === 0 && (
                  <div className='border-b-[0.5px] border-[#e5e5e6] h-[48px] flex items-center justify-center px-[8px] py-[4px] bg-gray-50'></div>
                )}
              </div>
              ...
              <div className='flex flex-col w-[134px] border-r border-[#e5e5e6]'>
                ...
                {store.specifications.map((spec) => (
                  <div
                    key={`action-${spec.id}`}
                    className='border-b-[0.5px] border-[#e5e5e6] h-[48px] flex items-center justify-center gap-[12px] px-[8px] py-[4px]'
                  >
                    <button
        ...
        <div className='flex flex-col gap-[8px] w-full mt-4'>
          ...
          <QuillEditor
            ...
            value={store.keyFeatures}
            onChange={(val) => store.setField('keyFeatures', val)}
          />
        </div>
        ...
        <div className='flex flex-col gap-[8px] w-full mt-4'>
          ...
          <QuillEditor
            ...
            value={store.detailedDescription}
            onChange={(val) => store.setField('detailedDescription', val)}
          />
        </div>

        {/* Returns Section */}
        <div className='flex flex-col md:flex-row gap-4 md:gap-[24px] w-full pt-[24px] mt-4 border-t border-[#e5e5e6]'>
          <div className='flex flex-col gap-[8px] flex-[0.33] w-full relative'>
            <p className='font-normal leading-[1.2] text-[16px] text-black'>Return Policy</p>
            <div className='bg-white border border-[#e5e5e6] flex items-center justify-between overflow-clip px-[12px] py-[10px] rounded-[2px] relative w-full'>
              <select
                ...
                onChange={(e) => store.setField('returnPolicy', e.target.value)}
              >
          ...
          <div className='flex flex-col gap-[8px] flex-[0.67] w-full'>
            <p className='font-normal leading-[1.2] text-[16px] text-black'>Return terms</p>
            <div className='bg-white border border-[#e5e5e6] flex items-start overflow-clip px-[12px] py-[10px] rounded-[2px] relative w-full'>
              <textarea
                value={store.returnTerms}
                onChange={(e) => store.setField('returnTerms', e.target.value)}
        ...
        <div className='w-full pt-[24px] mt-4 border-t border-[#e5e5e6]'>
