...
import { ChevronDown, Edit2, Trash2, Image as ImageIcon, Eye, ChevronLeft, ChevronRight, Plus, X } from 'lucide-react';
        ...
        <div className="flex flex-col md:flex-row gap-4 md:gap-[24px] items-start md:items-center justify-between relative w-full p-4 md:px-[24px] md:py-[16px]">
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center gap-3 w-full md:w-auto justify-between md:justify-end">
            ...
            <select
              ...
              className="bg-white border border-[#e5e5e6] h-[36px] px-3 rounded-[2px] text-[13px] text-black outline-none focus:border-[#f09000] cursor-pointer"
            >
            ...
            <button
              ...
              className="bg-[#f09000] hover:bg-[#e08600] text-white font-medium text-[13px] h-[36px] px-4 rounded-[2px] flex items-center gap-1.5 transition-colors cursor-pointer shrink-0 shadow-sm"
            >
          ...
          <div className="w-full overflow-x-auto">
            <div className="min-w-[1100px] w-full flex flex-col">
              ...
              <div className="bg-[#f2f2f3] border-b border-[#e5e5e6] flex items-center px-6 py-[12px]">
                <div className="w-[80px] shrink-0 text-[14px] text-[#42454d] font-semibold">Banner</div>
                <div className="w-[120px] shrink-0 text-[14px] text-[#42454d] font-semibold">Deal ID</div>
                <div className="flex-[1.5] text-[14px] text-[#42454d] font-semibold">Deal Name</div>
                <div className="flex-1 text-[14px] text-[#42454d] font-semibold">Banner Heading</div>
                <div className="w-[140px] shrink-0 text-[14px] text-[#42454d] font-semibold">% Discount Range</div>
                <div className="w-[130px] shrink-0 text-[14px] text-[#42454d] font-semibold">Applied Categories</div>
                <div className="w-[100px] shrink-0 text-[14px] text-[#42454d] font-semibold text-center">Status</div>
                <div className="w-[120px] shrink-0 text-[14px] text-[#42454d] font-semibold text-center">Actions</div>
              </div>
              ...
              {isLoading ? (
                ...
                <div className="p-12 text-center flex flex-col items-center gap-3">
                  <p className="text-[15px] font-medium text-[#42454d]">No deals or promotions found.</p>
                  <p className="text-[13px] text-[#848995]">Click the button below to add your first deal banner!</p>
                  <button
                    onClick={onAddNewDeal}
                ...
                deals.map((deal) => (
                  <div key={deal.id} className="border-b border-[#e5e5e6] flex items-center px-6 py-3.5 hover:bg-gray-50 transition-colors">
                    
                    {/* Banner Image */}
                    <div className="w-[80px] shrink-0">
                      <div className="size-[42px] overflow-hidden rounded-sm border border-[#e5e5e6] flex items-center justify-center relative shadow-xs" style={{ backgroundColor: deal.bannerBgColor || '#ffffff' }}>
                        {deal.bannerImage ? (
                          <img src={getFullImageUrl(deal.bannerImage)} alt={deal.title} className="w-full h-full object-cover" />
                        ) : (
                    ...
                    {/* ID */}
                    <div className="w-[120px] shrink-0 text-[12px] text-[#848995] font-mono truncate pr-2">
                    ...
                    {/* Discount Range */}
                    <div className="w-[140px] shrink-0 text-[13px] text-[#f09000] font-semibold">
                      {deal.discountStartPercent ?? 0}% - {deal.discountEndPercent ?? 0}%
                    </div>
                    
                    {/* Categories count */}
                    <div className="w-[130px] shrink-0 text-[12px] text-[#42454d]">
                    ...
                    <div className="w-[110px] shrink-0 flex items-center justify-center">
                      <button
                        ...
                        className={`inline-flex items-center gap-1.5 text-[12px] font-medium border rounded-[2px] px-2.5 py-1 transition-all cursor-pointer hover:shadow-xs active:scale-95 disabled:opacity-50 ${
                          deal.isActive 
                            ? 'border-[#c6e5d0] bg-[#eef8f0] text-[#229a4e] hover:bg-[#e2f3e5]' 
                            : 'border-[#d0e1ff] bg-[#f0f5ff] text-[#165dd0] hover:bg-[#e4edff]'
                        }`}
                      >
                        <span className={`size-1.5 rounded-full ${deal.isActive ? 'bg-[#229a4e]' : 'bg-[#165dd0]'}`} />
                        {deal.isActive ? 'Active' : 'Disabled'}
                      </button>
                    </div>
                    
                    {/* Action Buttons */}
                    <div className="w-[120px] shrink-0 flex items-center justify-center gap-3">
                      <button
      ...
      {meta && meta.totalPage > 1 && (
        <div className="flex items-center justify-between w-full px-2 mt-2">
          <p className="text-[13px] text-[#848995]">
            Showing total {meta.total} deals
          </p>
          <div className="flex items-center gap-1">
            <button 
              onClick={() => setPage((prev) => Math.max(prev - 1, 1))}
              disabled={page === 1}
              className="flex items-center gap-1 px-3 py-1 text-[13px] text-[#848995] hover:text-black disabled:opacity-50 cursor-pointer border border-[#e5e5e6] rounded-[2px]"
            >
            ...
            <button 
              ...
              className="flex items-center gap-1 px-3 py-1 text-[13px] text-black hover:bg-gray-50 border border-[#e5e5e6] rounded-[2px] disabled:opacity-50 cursor-pointer"
            >
              Next
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg p-6 max-w-xl w-full flex flex-col gap-4 shadow-xl relative">
            <div className="flex items-center justify-between border-b border-[#e5e5e6] pb-3">
              ...
              <button onClick={() => setViewModalDeal(null)} className="text-gray-500 hover:text-black p-1 cursor-pointer">
                <X size={18} />
              </button>
            </div>

            {/* Banner Preview */}
            <div className="w-full h-44 rounded-xs overflow-hidden relative flex flex-col justify-center px-6" style={{ backgroundColor: viewModalDeal.bannerBgColor || '#ffffff' }}>
              <h2 className="text-black text-2xl font-bold">{viewModalDeal.bannerHeading || viewModalDeal.title}</h2>
              <p className="text-[#42454d] text-lg mt-1">{viewModalDeal.bannerSubheading}</p>
              {viewModalDeal.bannerImage && (
                <img src={getFullImageUrl(viewModalDeal.bannerImage)} alt="Banner Graphic" className="absolute right-4 top-1/2 -translate-y-1/2 h-32 object-contain" />
              )}
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 text-[13px] border-t border-[#e5e5e6] pt-3">
              <div>
                ...
                <p className="font-semibold text-black">{viewModalDeal.discountStartPercent ?? 0}% - {viewModalDeal.discountEndPercent ?? 0}%</p>
              </div>
              <div>
                <span className="text-[#848995]">Status:</span>
                <p className="font-semibold text-black">{viewModalDeal.isActive ? 'Active' : 'Disabled'}</p>
              </div>
              <div>
                <span className="text-[#848995]">Start Date:</span>
                <p className="font-medium text-black">{viewModalDeal.startDate ? new Date(viewModalDeal.startDate).toLocaleDateString() : 'N/A'}</p>
              </div>
              <div>
                <span className="text-[#848995]">End Date:</span>
                <p className="font-medium text-black">{viewModalDeal.endDate ? new Date(viewModalDeal.endDate).toLocaleDateString() : 'N/A'}</p>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-md rounded border border-[#E5E5E6] bg-white p-6 shadow-xl relative flex flex-col gap-4">
            ...
            <p className="text-sm text-[#42454D] leading-relaxed">
              Are you sure you want to delete this deal? This action cannot be undone and will permanently remove the deal banner and promotion configuration.
            </p>
