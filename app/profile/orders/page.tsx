    ...
    items: [
      {
        ...
        seller: 'keyb co',
        price: 75.0,
        imageUrl: 'https://images.unsplash.com/photo-1587829741301-dc798b83add3?q=80&w=150',
      },
    ],
  },
  {
    ...
    items: [
      {
        ...
        seller: 'displays inc',
        price: 299.99,
        imageUrl: 'https://images.unsplash.com/photo-1527443224154-c4a3942d3acf?q=80&w=150',
      },
    ],
  },
  {
    ...
    items: [
      {
        ...
        seller: 'tech store',
        price: 45.0,
        imageUrl: 'https://images.unsplash.com/photo-1527814050087-3793815479bd?q=80&w=150',
      },
    ],
  },
];
...
export default function ProfileOrdersPage() {
  ...
  const filteredOrders = useMemo(() => {
    return orders.filter((order) => activeTab === 'All Orders' || order.status === activeTab);
  }, [orders, activeTab]);
    ...
    <div className='flex flex-col gap-6 items-start w-full text-left'>
          ...
          {TABS.map((tab) => {
            ...
            return (
              <button
                ...
                className={`flex items-center p-2 transition-all cursor-pointer rounded-xs ${
      ...
      <div className='flex flex-col gap-4.5 w-full'>
        {paginatedOrders.length > 0 ? (
          paginatedOrders.map((order) => {
            ...
            return (
              <div
                key={order.id}
                className='border border-[#e5e5e6] p-6 flex flex-col gap-9 w-full'
              >
                ...
                >
                  ...
                  <div className='flex flex-col gap-2 items-start'>
                    <div className='flex gap-6 items-center'>
                      ...
                      >
                        <span
                          className={`font-['Open_Sans'] font-normal text-[12px] ${badgeText} leading-[1.3]`}
                        >
                          {order.status}
                        </span>
                      </div>
                  ...
                  <>
                    ...
                    <div className='flex flex-col gap-2.5 w-full'>
                      {order.items.map((item) => (
                        <div
                          key={item.id}
                          className='border-b border-[#e5e5e6] py-2 flex gap-4 items-center w-full'
                        >
                          <div className='w-25 h-25 shrink-0 relative rounded-xs overflow-hidden border border-gray-100 bg-gray-50'>
                            <Image
                              src={item.imageUrl}
                              alt={item.name}
                              fill
                      ...
                      {order.status === 'Ordered' && (
                        <>
                          <button
                            ...
                            className='border-[0.75px] border-[#cb1b41] rounded-xs px-4 py-3 min-w-20 flex items-center justify-center hover:bg-red-50 transition-colors cursor-pointer'
                          >
                          ...
                          <button
                            ...
                            className='bg-[#dec33a] border border-[#dec33a] rounded-xs px-4 py-3 min-w-20 flex items-center justify-center hover:bg-[#c9b030] transition-colors cursor-pointer'
                          >
        ...
        {filteredOrders.length > 0 && (
          <div className='flex gap-1 items-center justify-center w-full pt-4 pb-2'>
            <button
              type='button'
              onClick={() => setCurrentPage((p) => Math.max(1, p - 1))}
              disabled={currentPage === 1}
              className='flex gap-1 items-center justify-center min-w-20 px-3 py-2 rounded-[6px] hover:bg-gray-100 cursor-pointer transition-colors disabled:opacity-50 disabled:cursor-not-allowed'
            >
            ...
            {Array.from({ length: totalPages }, (_, i) => i + 1).map((pageNum) => (
              <button
                ...
                className={cn(
                  'flex items-center justify-center w-10 h-10 rounded-xs cursor-pointer transition-colors',
                  currentPage === pageNum
