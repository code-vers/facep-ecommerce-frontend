...
const RECENTLY_VIEWED_ITEMS: CarouselProduct[] = [
  ...
  },
  {
    ...
    title: "Gaming Setup",
    imageSrc:
      "https://images.unsplash.com/photo-1602143407151-7111542de6e8?q=80&w=300",
    imageAlt: "Gaming Setup",
    rating: 4.5,
    reviewCount: "624+",
    price: "$299.99",
...
const isStepActive = (
  orderStatus: string,
  stepKey: (typeof STATUS_ORDER)[number],
) => {
  if (orderStatus === "Returned") return true;
  const orderStatusIndex = STATUS_ORDER.indexOf(
    orderStatus as (typeof STATUS_ORDER)[number],
  );
...
export default function ReturnsAndOrdersPage() {
  ...
  const [activeTab, setActiveTab] =
    useState<(typeof FILTER_TABS)[number]>("All Orders");
  ...
  useEffect(() => {
    if (activeTrackOrder) document.body.style.overflow = "hidden";
    else document.body.style.overflow = "";
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === "Escape") setActiveTrackOrder(null);
    };
  ...
  const fetchOrders = async (page: number, status: string) => {
    const url = new URL(
      `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000/api/v1"}/checkout/my-orders`,
    );
  ...
  const { data: apiData, isLoading } = useQuery({
    queryKey: ["orders", currentPage, activeTab],
    queryFn: () => fetchOrders(currentPage, activeTab),
  ...
  const orders: Order[] = useMemo(() => {
    ...
    return apiData.data.map((dbOrder: any) => {
      let uiStatus = "Ordered";
      if (dbOrder.status === "PROCESSING") uiStatus = "Packed";
      if (dbOrder.status === "SHIPPED") uiStatus = "Shipped";
      if (dbOrder.status === "DELIVERED") uiStatus = "Delivered";
      if (dbOrder.status === "CANCELLED") uiStatus = "Returned";

      return {
        id: dbOrder.orderNumber,
        status: uiStatus as any,
        placedDate: new Date(dbOrder.createdAt).toLocaleDateString("en-US", {
          year: "numeric",
          month: "long",
          day: "numeric",
        }),
            ...
            <div className="py-10">
              <div className="flex flex-col gap-8 relative max-w-150 mx-auto">
                {TRACKING_STEPS.map((step, index) => {
                  ...
                  const { date, time } = getStepDateTime(
                    activeTrackOrder.placedDate,
                    index,
                  );
